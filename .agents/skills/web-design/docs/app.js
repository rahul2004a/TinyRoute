...
function initDomeGallery(root) {
  ...
  // ---- geometry (parameters matching the original DomeGallery) ----
  const SEGMENTS = 35;        // Latitude/longitude divisions across the hemisphere
  const TILE_SX = 2;
  const TILE_SY = 2;
  const MAX_VERT = 5;         // Original maxVerticalRotationDeg = 5
  const DRAG_SENS = 20;       // Original dragSensitivity = 20
  const DRAG_DAMP = 0.6;
  const AUTO_SPIN = 0.035;    // Gentle auto-rotation: 0.035deg per frame ≈ 2.1 deg/s

  // Synchronize CSS variables (drives --rot-y / --rot-x / --item-width / --item-height)
  root.style.setProperty('--segments-x', SEGMENTS);
  root.style.setProperty('--segments-y', SEGMENTS);

  // Image palette (each <img> is the visual source for a tile; JavaScript repeats them across the hemisphere)
  ...
  // Generate staggered grid coordinates (exactly matching the original)
  // xCols: [-34, -32, ..., 32, 34]  (35 columns)
  // evenYs: [-4, -2, 0, 2, 4]; oddYs: [-3, -1, 1, 3, 5]
  const xCols = Array.from({ length: SEGMENTS }, (_, i) => -SEGMENTS - 1 + 2 * i + 1);
  // The expression above is awkward. Follow the original directly: -37 + i*2 skews left, so use centered 2-unit spacing
  const xColsCentered = [];
  ...
  xColsCentered.forEach((x, c) => {
    ...
    ys.forEach((y) => {
      ...
      // Cycle through the image palette (the same image appears multiple times on the sphere)
  ...
  // ---- dragging ----
  let dragging = false;
  let autoPausedUntil = 0;      // Pause auto-rotation briefly after dragging
  ...
  const onUp = (e) => {
    ...
    autoPausedUntil = performance.now() + 1500; // Resume auto-rotation 1.5s after release
  ...
  // ---- gentle auto-rotation (main loop) ----
  let visible = true;
  const autoTick = () => {
    const now = performance.now();
...
document.querySelectorAll('.copy-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    const doneText = (lang === 'en') ? 'Copied ✓' : 'Copied ✓';
...
const i18n = {
  ...
  zh: {
    brand: 'web-design',
    'hero.eyebrow': 'THE DESIGN SKILL FOR CLAUDE CODE',
    'hero.before': 'Spec', 'hero.spec': 'first', 'hero.comma': ',',
    'hero.before2': 'then', 'hero.code': 'code', 'hero.period': '.',
    'hero.sub1': 'Feed it a PRD, reference URL or screenshot.',
    'hero.sub2': 'It produces a readable, editable, portable <em>DESIGN.md</em> first —',
    'hero.sub3': 'then generates web code where UI/UX, visuals, motion and responsive behavior all meet the bar.',
    'hero.cta1': 'Install now', 'hero.cta2': 'Why this order →',
    'step.1': 'Input · PRD / URL / screenshot', 'step.2': 'Produce · DESIGN.md spec', 'step.3': 'Generate · 100-score code',
    'why.eyebrow': 'WHY DESIGN.MD', 'why.title': 'Why a spec comes first',
    'why.sub': 'AI writes code directly vs spec first, code second — the results are in different leagues.',
    'why.bad.tag': 'AI codes directly',
    'why.bad.l1': '× Colors drift every generation',
    'why.bad.l2': '× Type scale lives in AI memory',
    'why.bad.l3': '× Motion tier changes by mood',
    'why.bad.l4': '× New tool, new conversation, start over',
    'why.good.tag': 'DESIGN.md first, then code',
    'why.good.l1': '✓ Colors, type, spacing all named',
    'why.good.l2': '✓ Consistent across pages',
    'why.good.l3': '✓ Portable across AI tools',
    ...
    'inside.title': 'A full design asset vault',
    'inside.sub': 'The SKILL ships with its own reference library — drag to spin, hover to pause.',
    'rolling.hint': 'Drag or scroll to spin · hover to pause',
    'art.1.title': '58 brand presets',
    'art.1.body': 'Linear · Stripe · Apple · Supabase · Vercel · Figma · Cursor · …',
