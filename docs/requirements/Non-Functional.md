...
| NFR-MNT-03 | Documented decisions | Significant technical choices are recorded as short ADRs in `docs/adr/`. | Should | V1 |
...
| NFR-TST-02 | Coverage | At least 70% line coverage on application code, with the critical paths fully covered. | Should | V1 |
| NFR-TST-03 | Load test script | A committed script reproduces the throughput and latency measurements. | Should | V1 |
...
| NFR-PRV-03 | Anonymous analytics | Click analytics store no visitor IP address or identifier that could single out a person. | Must | V1 |
| NFR-PRV-04 | Account deletion | Deleting an account removes personal data from the live datastore within 24 hours; it ages out of backups per NFR-BAK-05. | Should | V1 |
